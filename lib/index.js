// 设置背景 (◕‿◕) — 宿主半身（永久版 v3）
// 提供：/anime-bg/ 图片路由、/anime-bg-thumb/ 缩略图路由、/anime-bg-api/* JSON API
...
export function apply(ctx) {
  ctx.inject(['webServer', 'fs'], (ready) => {
    ...
    const statePath = `${wsRoot}\\.anime-bg-state.json`
    const thumbDir = join(wsRoot, '.anime-bg-thumbs')
    let currentDir = 'E:\\images\\焦茶'

    // ── 缩略图引擎：PowerShell System.Drawing 缩放 + 磁盘缓存 + 并发队列 ──
    ...
    const generateThumb = (name) => new Promise((resolve, reject) => {
      ...
      const script = [
        ...
        "if(!(Test-Path $env:THUMB_DIR)){New-Item -ItemType Directory -Force -Path $env:THUMB_DIR | Out-Null}",
        "$img=[System.Drawing.Image]::FromFile($env:THUMB_SRC)",
        "$w=160; $h=[int]($img.Height*$w/$img.Width)",
        "$bmp=New-Object System.Drawing.Bitmap($w,$h)",
        "$g=[System.Drawing.Graphics]::FromImage($bmp)",
        "$g.InterpolationMode='HighQualityBicubic'",
        "$g.DrawImage($img,0,0,$w,$h)",
        "$bmp.Save($env:THUMB_DST,[System.Drawing.Imaging.ImageFormat]::Jpeg)",
        "$g.Dispose(); $bmp.Dispose(); $img.Dispose()",
      ].join('; ')
      execFile('powershell', ['-NoProfile', '-NonInteractive', '-EncodedCommand', psEncode(script)], {
        timeout: 30000,
        env: { ...process.env, THUMB_DIR: thumbDir, THUMB_SRC: srcPath, THUMB_DST: dstPath },
      }, (err) => (err ? reject(err) : resolve(dstPath)))
    })
    ...
    // GET /anime-bg-api/list
    ctx.effect(() => webServer.register({
      kind: 'exact',
      path: '/anime-bg-api/list',
      handler: async (req, res) => json(res, 200, await listImages()),
    }))

    // POST /anime-bg-api/set-dir  { path }
    ctx.effect(() => webServer.register({
      kind: 'exact',
      path: '/anime-bg-api/set-dir',
    ...
    // POST /anime-bg-api/save  { state }
    ctx.effect(() => webServer.register({
      kind: 'exact',
      path: '/anime-bg-api/save',
      handler: async (req, res) => {
        try {
          const raw = await readBody(req)
    ...
    // GET /anime-bg-api/load
    ctx.effect(() => webServer.register({
      kind: 'exact',
      path: '/anime-bg-api/load',
      handler: async (req, res) => {
        try {
          const target = await fs.resolve(statePath)
    ...
    // GET /anime-bg-thumb/<name> — 缩略图（PowerShell 生成，磁盘缓存）
    ctx.effect(() => webServer.register({
      kind: 'prefix',
      path: '/anime-bg-thumb',
      handler: async (req, res) => {
        try {
          const name = validName(req.url ?? '/', '/anime-bg-thumb/')
          if (name === null) {
    ...
    // GET /anime-bg/<name> — 图片
    ctx.effect(() => webServer.register({
      kind: 'prefix',
      path: '/anime-bg',
      handler: async (req, res) => {
        try {
          const name = validName(req.url ?? '/', '/anime-bg/')
          if (name === null) {
            res.writeHead(404, { 'Content-Type': 'text/plain' })
            res.end('bad name')
            return
