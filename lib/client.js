  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      disposes.push(insertCss(`
        ...
        /* dsh 0.1.2-rc.1: assistant text renders inside .hWmORq_root with no
           surface of its own — draw the translucent bubble here (same variable
           the product uses for the user-side bubble). */
        .hWmORq_root {
          background: var(--dsw-specific-bubble, rgba(20, 22, 28, 0.75));
          border-radius: 22px;
          padding: 10px 16px;
        }
        /* 消息时间戳（操作条左侧时钟）：半透明画布上保证可读 */
        .xzv4MW_timeStart, .xzv4MW_timeEnd {
          color: var(--dsw-alias-label-primary) !important;
          text-shadow: 0 1px 2px rgba(255, 255, 255, 0.65), 0 0 6px rgba(255, 255, 255, 0.35);
        }
        html:root body[data-ds-dark-theme] .xzv4MW_timeStart,
        html:root body[data-ds-dark-theme] .xzv4MW_timeEnd {
          text-shadow: 0 1px 2px rgba(0, 0, 0, 0.65), 0 0 6px rgba(0, 0, 0, 0.4);
        }
        /* 气泡（用户侧产品自带 .Sixlwa_bubble + 助手侧 .hWmORq_root）内部文字不叠加阴影 */
        .hWmORq_root, .hWmORq_root *,
        .Sixlwa_bubble, .Sixlwa_bubble * {
          text-shadow: none;
        }
      `))
      restore()
