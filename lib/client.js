  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      disposes.push(insertCss(`
        ...
        /* 高对比阴影模式（通用兜底）：聊天列内、气泡外的所有文字统一叠加阴影。
           text-shadow 可继承，只需在列根上声明一次；未单独写阴影的元素（统计条、
           压缩/上下文行、日期、状态文字等）都会自动获得同款高对比描边 */
        .pI_x6G_centerCol {
          text-shadow: 0 1px 2px rgba(255, 255, 255, 0.65), 0 0 6px rgba(255, 255, 255, 0.35);
        }
        html:root body[data-ds-dark-theme] .pI_x6G_centerCol {
          text-shadow: 0 1px 2px rgba(0, 0, 0, 0.65), 0 0 6px rgba(0, 0, 0, 0.4);
        }
        /* 气泡（助手/自己消息）内部文字：清掉继承阴影，保持原有气泡内观感 */
        .Sxvs8a_root, .Sxvs8a_root *,
        .gdEzaW_bubble, .gdEzaW_bubble * {
          text-shadow: none;
        }
        /* 运行状态行（如 "Deep diving..."）是渐变剪贴文字：text-shadow 会叠在字形
           上面把它盖暗（普通实心文字不受影响），所以改用 filter: drop-shadow——
           阴影只描在字形轮廓外，渐变字保持明亮 */
        .Md3f7G_turnStatus, .Md3f7G_turnStatus * {
          text-shadow: none;
        }
        .Md3f7G_turnStatus {
          filter: drop-shadow(0 1px 2px rgba(255, 255, 255, 0.65)) drop-shadow(0 0 6px rgba(255, 255, 255, 0.35));
        }
        html:root body[data-ds-dark-theme] .Md3f7G_turnStatus {
