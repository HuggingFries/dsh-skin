window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const api = {
      list: () => fetch("/anime-bg-api/list").then((r) => r.json()),
      setDir: (path) => fetch("/anime-bg-api/set-dir", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ path }) }).then((r) => r.json()),
      save: (state) => fetch("/anime-bg-api/save", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ state }) }).then((r) => r.json()).catch(() => ({ ok: false })),
      load: () => fetch("/anime-bg-api/load").then((r) => r.json()),
    };
    ...
    const bgCss = (name, dir, s) => {
      return `
        .pI_x6G_centerCol {
          background-image: url("/anime-bg/${name}?d=${encodeURIComponent(dir)}") !important;
    ...
    const applyTheme = (tokens) => {
      ...
      if (themeRef != null) {
        ...
        disposeThemeTokens = themeRef.overrideTokens("anime-bg-theme", pair)
      }
    ...
    const togglePlay = () => { if (playing) stopSlide(); else startFavPlay(); notify() }
    ...
    function BackgroundPicker() {
      ...
      const thumb = (n, size) => React.createElement("img", {
        src: "/anime-bg-thumb/" + n + "?d=" + encodeURIComponent(dir),
        onError: (e) => {
          const el = e.currentTarget
          if (el && el.src.indexOf("/anime-bg-thumb/") >= 0) {
            el.src = "/anime-bg/" + n + "?d=" + encodeURIComponent(dir)
          }
        },
        decoding: "async",
        loading: "lazy",
        ...
        ? React.createElement("div", {
            ...
            React.createElement("div", {
              style: {
                ...
                backgroundImage: "url(\"/anime-bg/" + name + "?d=" + encodeURIComponent(dir) + "\")",
      ...
      return React.createElement("div", {
        ...
        themeMode === "auto" && name
          ? React.createElement("div", { style: { display: "none" } },
              ...
              React.createElement("img", { ref: (el) => { els.img = el }, src: "/anime-bg/" + name + "?d=" + encodeURIComponent(dir), onLoad: onDetectLoad }))
          : null,
    ...
    function apply(ctx) {
      ...
      slots.inject("settings.section", () => slots.register(
        { name: "settings.section", id: "anime-bg", order: 30, label: "设置背景 (◕‿◕)" },
