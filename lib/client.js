window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // timer 是 Cordis context mixin 而非注册服务，ctx.get 可能拿不到；逐级兜底
      if (timerRef == null && ctx.timer != null && typeof ctx.timer.interval === "function") timerRef = ctx.timer
      if (timerRef == null) {
      ...
      disposes.push(slots.inject("settings.section", () => slots.register(
        { name: "settings.section", id: "dsh-skin", order: 30, label: "设置背景 (◕‿◕)" },
