window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 兼容探针:dsh 升级后若适配类已失效,给出明确指引而不是无声失效。 */
    function probeAdaptation() {
      try {
        let tries = 0
        const timer = window.setInterval(() => {
          tries += 1
          const found = Object.values(ADAPT).some((cls) => document.querySelector("." + cls) !== null)
          if (found || tries > 12) {
            window.clearInterval(timer)
            if (!found) {
              console.warn(
                "[dsh-skin] 兼容探针:页面中未发现适配类(" +
                  Object.values(ADAPT).join(", ") +
                  "),当前 dsh UI 结构可能已变化 —— 运行 node tools/adapt.mjs 重新采集哈希后升级皮肤。"
              )
            }
          }
        }, 2500)
