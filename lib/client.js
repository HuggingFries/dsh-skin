window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 由 ADAPT 常量在运行时生成注入 CSS —— 哈希类名只在此文件出现一次。 */
    function buildAdaptCss(a) {
      const lines = [
        "html { background-color: #0a0c10 !important; }",
        "/* 助手气泡壳(rc.1 起产品不提供助手消息外壳,由皮肤补画) */",
        "." + a.assistantRoot + " {",
        "  background: var(--dsw-specific-bubble, rgba(20, 22, 28, 0.75));",
        "  border-radius: 22px;",
        "  padding: 10px 16px;",
        "}",
        "/* 全局可读性阴影:消息列内、气泡外的所有文字统一叠加描边 */",
        "." + a.column + " {",
        "  text-shadow: 0 1px 2px rgba(255, 255, 255, 0.65), 0 0 6px rgba(255, 255, 255, 0.35);",
        "}",
        "html:root body[data-ds-dark-theme] ." + a.column + " {",
        ...
        "  text-shadow: 0 1px 2px rgba(255, 255, 255, 0.65), 0 0 6px rgba(255, 255, 255, 0.35);",
        "}",
        "html:root body[data-ds-dark-theme] ." + a.timeStart + ",",
        "html:root body[data-ds-dark-theme] ." + a.timeEnd + " {",
        "  text-shadow: 0 1px 2px rgba(0, 0, 0, 0.65), 0 0 6px rgba(0, 0, 0, 0.4);",
        "}",
        "/* 运行状态行是渐变剪贴文字:text-shadow 会盖暗字形,改用 drop-shadow */",
        "." + a.turnStatus + ", ." + a.turnStatus + " * {",
        "  text-shadow: none;",
        "}",
        "." + a.turnStatus + " {",
        "  filter: drop-shadow(0 1px 2px rgba(255, 255, 255, 0.65)) drop-shadow(0 0 6px rgba(255, 255, 255, 0.35));",
        "}",
        "html:root body[data-ds-dark-theme] ." + a.turnStatus + " {",
        "  filter: drop-shadow(0 1px 2px rgba(0, 0, 0, 0.65)) drop-shadow(0 0 6px rgba(0, 0, 0, 0.4));",
        "}",
        "/* 气泡内部清阴影 */",
        "." + a.assistantRoot + ", ." + a.assistantRoot + " *,",
        "." + a.bubble + ", ." + a.bubble + " * {",
        "  text-shadow: none;",
        "}",
      ]
    ...
    /** 皮肤自己注入的样式标签（带 dataset 标记），探针统计时跳过，避免误判。 */
    const SKIN_STYLE_DATASET = "dshSkinProbeSkip"

    /** 收集应用样式表里的全部选择器文本（跳过皮肤自身的样式）。 */
    function appSelectors() {
      ...
      for (const sheet of sheets) {
        try {
          ...
          if (owner && owner.dataset && owner.dataset[SKIN_STYLE_DATASET] === "1") continue
          const rules = sheet.cssRules || []
          for (const rule of rules) {
            if (rule && typeof rule.selectorText === "string") out.push(rule.selectorText)
          }
        } catch (_) { /* 跨源样式表不可读，忽略 */ }
      }
      return out
    ...
    /** 兼容探针（加固版）：逐键检查 ADAPT 类名是否仍存在于应用样式表中。
        判定依据是“类名是否还被 dsh 界面样式引用”，与当前会话渲染了哪些元素无关，
        因此单个类名失效也会被发现；失效时给出页面可见告警而不是静默。 */
    function probeAdaptation() {
      try {
        let tries = 0
        const timer = window.setInterval(() => {
          ...
          const missing = []
          for (const key of Object.keys(ADAPT)) {
            const cls = ADAPT[key]
            if (typeof cls !== "string" || cls === "") continue
            const re = new RegExp("\\.(" + cls + ")(?=[^A-Za-z0-9_-]|$)")
            if (!selectors.some((s) => re.test(s))) missing.push(key + "=" + cls)
          }
          if (missing.length === 0 || tries > 12) {
            window.clearInterval(timer)
            if (missing.length === 0) {
              hideAdaptAlert()
            } else {
              console.warn("[dsh-skin] 兼容探针：以下适配类已不在当前 dsh 界面样式中：", missing.join(", "))
              console.warn("[dsh-skin] 请运行 node tools/adapt.mjs 重新采集哈希类名并更新 ADAPT，然后重载页面。")
              showAdaptAlert(missing)
            }
          }
        }, 2000)
      } catch (_) { /* ignore */ }
    ...
    /** 页面右下角可见告警（控制台对普通用户不可见）。 */
    function showAdaptAlert(missing) {
      try {
        let el = document.querySelector("#dsh-skin-adapt-alert")
        if (!el) {
          el = document.createElement("div")
          el.id = "dsh-skin-adapt-alert"
          el.style.cssText = "position:fixed;right:12px;bottom:12px;z-index:2147483000;max-width:360px;background:#7a1f1f;color:#fff;font:12px/1.6 system-ui,sans-serif;padding:10px 14px;border-radius:10px;box-shadow:0 6px 24px rgba(0,0,0,.35);white-space:pre-wrap;cursor:pointer;"
          el.title = "点击关闭"
          el.addEventListener("click", () => { try { el.remove() } catch (_) { /* ignore */ } })
          document.body.appendChild(el)
        }
        el.textContent = "[dsh-skin] dsh 更新后界面类名已变，以下效果可能失效：\n" +
          missing.join("\n") + "\n\n运行 node tools/adapt.mjs 重新采集并更新 ADAPT 后刷新页面。"
      } catch (_) { /* ignore */ }
    }
    function hideAdaptAlert() {
      try {
        const el = document.querySelector("#dsh-skin-adapt-alert")
        if (el) el.remove()
      } catch (_) { /* ignore */ }
    }
    ...
    const insertCss = (css) => {
      ...
      tag.textContent = css;
      tag.dataset[SKIN_STYLE_DATASET] = "1";
    ...
    const setBackground = (name, dir, override) => {
      ...
      // 手动换图更流畅：预加载列表中的相邻两张图
      try {
        const idx = imageList.findIndex((im) => im.name === name)
        if (idx >= 0 && imageList.length > 1) {
          for (const off of [-1, 1]) {
            const nb = imageList[(idx + off + imageList.length) % imageList.length]
            if (nb && nb.name !== name) {
              const im = new Image()
              im.src = "/dsh-skin/" + nb.name + "?d=" + encodeURIComponent(dir)
            }
          }
        }
      } catch (_) { /* ignore */ }
    ...
    const applyTheme = (tokens) => {
      ...
      // 复用单个 style 标签，只更新 textContent（不再每次删旧建新）
      if (!themeCssTag || !themeCssTag.parentNode) {
        themeCssTag = document.createElement("style")
        document.head.appendChild(themeCssTag)
      }
      themeCssTag.textContent = "html:root body { " + l + " } html:root body[data-ds-dark-theme] { " + d + " }"
    }
    const reapplyTheme = () => {
    ...
    const startFavPlay = (immediate) => {
      ...
      if (timerRef != null) {
        slideTimer = timerRef.interval(() => {
          ...
          favIndex = (favIndex + 1) % favorites.length
          // 预加载下一张，切图更顺滑（提前解码）
          const pre = favorites[favIndex]
          if (pre) {
            try {
              const im = new Image()
              im.src = "/dsh-skin/" + pre.name + "?d=" + encodeURIComponent(currentDir)
            } catch (_) { /* ignore */ }
          }
        }, slideIntervalMs)
      }
    ...
    function BackgroundPicker() {
      ...
      const measure = React.useCallback(() => {
        ...
        const frame = document.querySelector("." + ADAPT.frame)
        const col = document.querySelector("." + ADAPT.centerCol)
        if (!frame || !col) return
        const fr = frame.getBoundingClientRect()
        const cr = col.getBoundingClientRect()
        const side = document.querySelector("." + ADAPT.sidebar)
        const det = document.querySelector("." + ADAPT.details)
    ...
    function apply(ctx) {
      ...
      const disposes = []
      disposes.push(insertCss(buildAdaptCss(ADAPT)))
      probeAdaptation()
      restore()
      ...
      disposes.push(() => { if (themeCssTag && themeCssTag.parentNode) themeCssTag.parentNode.removeChild(themeCssTag) })
