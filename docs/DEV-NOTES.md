...
> 插件当前版本：固化版 v3（永久插件），源码位置：`E:\dsh\anime-bg\`（经 junction 挂在 `C:\Users\zxy\.dsh\profiles\web\node_modules\anime-bg`）
...
- 根治（已部署）：固化插件包 `C:\Users\zxy\.dsh\profiles\web\node_modules\anime-bg\`（package.json + lib/index.js 宿主半身 + lib/client.js 客户端半身，`window.__ModuleLoader__.load` 官方格式），并在 `profiles\web\cordis.patch.yml` 注册 `- id: anime-bg / name: 'anime-bg'`。重启 dsh web 后生效；动态版 anibg-1 随重启自然消失，不冲突。
- 备注：状态文件实际位于 dsh 进程启动目录（当前为 C:\Users\zxy\.anime-bg-state.json，按 sandboxPolicy.workspaceRoot 解析）；从不同目录启动 dsh 会读不到旧状态。
- 回滚：若启动失败，把 cordis.patch.yml 改回 `[]` 并删除 anime-bg 目录。
...
- 描述：重启后能进但插件不生效——探针显示 /anime-bg-api/* 与 /anime-bg/* 全部落到 SPA 兜底（宿主路由未注册），客户端 bundle 正常服务（包解析 OK）。
